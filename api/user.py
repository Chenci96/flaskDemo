...
def user_register():
    ...
    sex = request.json.get("sex", 0)  # 性别，默认为0(男性)
    telephone = request.json.get("telephone", "")  # 手机号
    ...
    if username and password and telephone: # 注意if条件中 "" 也是空, 按False处理
        ...
        elif not (len(str(telephone)) == 11 and re.match("^1[3,5,7,8]\d{9}$", str(telephone))):
...
def user_login():
    ...
    if username and password:   # 注意if条件中空串 "" 也是空, 按False处理
        ...
        print(f'md5_password:{md5_password}')
